...
Public Class frmEmployees
        ...
        If MessageBox.Show("Are you sure you want to continue?", "Warning", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = vbYes Then
            For Each rw As DataGridViewRow In MyDataGridView.SelectedRows
                If Not rw.Cells("Username").Value Is Nothing Then
                    com.CommandText = "insert into tblaccounts set fullname='" & rw.Cells("Fullname").Value.ToString & "', designation='" & UCase(rchar(rw.Cells("com_position").Value.ToString)) & "', officeid='" & globalofficeid & "', username='" & UCase(rw.Cells("Username").Value.ToString) & "',password='" & EncryptTripleDES(UCase(rw.Cells("Username").Value.ToString) + "1234") & "',datereg=current_timestamp;" : com.ExecuteNonQuery()
                End If
            Next
    ...
    Public Sub loadEmployees()
        ...
        msda = New MySqlDataAdapter("select emp_id as 'Username', Fullname, com_branch as 'Branch',com_position  from tblemployees where (emp_id like '%" & txtSearch.Text & "%' or fullname like '%" & txtSearch.Text & "%' or com_branch like '%" & txtSearch.Text & "%' or com_position like '%" & txtSearch.Text & "%') and emp_id not in (select username from tblaccounts) order by fullname asc;", conn)
